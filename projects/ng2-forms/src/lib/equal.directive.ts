...
@Directive({
  selector:
    '[krEqual][formControlName],[krEqual][formControl],[krEqual][ngModel]',
  providers: [
    {
...
export class EqualDirective implements Validator {
  ...
  constructor(
    @Attribute('krEqual') public krEqual: string,
    @Attribute('krRepeat') public retype?: string
  ) {}
  ...
  validate(c: AbstractControl): ValidationErrors | null {
    ...
    if (e && v !== e.value && this._isRetype) {
      return this._error;
    }

    // input values are not equal and self input is not retype
    if (e && v !== e.value && !this._isRetype) {
      e.setErrors({ ...e.errors, ...this._error });
    }
